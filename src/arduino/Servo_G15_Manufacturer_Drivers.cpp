...
G15::G15(USARTClass& sBus, byte ID, int cPin)	//, char ctrl)
{   
...
void G15::setControlPin(int pin)
{
  TxRx = pin;
}
...
word G15::send_packet(byte ID, byte inst, byte* data, byte param_len)	
{
	...
	for(i=0; i<packet_len;i++){
		serialBus->write(TxBuff[i]);
                //Serial.println(TxBuff[i],HEX);
	}
	serialBus->flush();
    ...
    if((ID != 0xFE) || (inst == iPING))
	{
		...
		byte readcount= serialBus->readBytes(Status, packet_len); 
